...
def part_two():
    ...
    for rucksack_string in input_lines('../inputs/day_three.txt'):
        if i == 2:
            # If it's the 3rd rucksack we're seeing, append it to the group:
            current_rucksack_group.append(rucksack_string)
            rucksack_groups.append(current_rucksack_group)
            i = 0
            current_rucksack_group = []
        else:
            current_rucksack_group.append(rucksack_string)
    ...
    # Now, work on each group:
    priority_sum = 0
    for rucksack_group in rucksack_groups:
        # What is the common element(s) between the three sacks?
        group_one, group_two, group_three = [set(rucksack) for rucksack in rucksack_group]
